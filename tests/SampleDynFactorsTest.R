##' MScPack
##' Description: Testing if SampleDynFactors are estimating factors correctly
...
for (i in 1:2){
  for (j in 0:3){
    ...
    plot(xcoord, factors[interval == j, i], type = "l", lwd = 2, 
         ...
         ylim = range(factors[interval == j, i],
                      factors.lwr[i, interval == j], 
...
# linear relations between true and estimated factors ---------------------

par(mfrow = c(1, 2), mar = c(2.1, 2.1, 2.1, 1.0))
...
for (i in 1:2){
  xcoord <- time.id
  plot(xcoord, factors[, i], type = "l", lwd = 2, 
       col = rgb(0, 0, 0.5, 0.5), bty = "l",
       main = paste("Factor", i), 
       ylim = range(factors.lwr[i, ], 
                    factors.upr[i, ]))
  points(xcoord, factors.mean[i, ], type = "l", lty = 2, col = 2)
